...
public:
    ...
    int compare(String* str) {
     ...
     const char* serialize() {
        ...
        s->serialize(size_);
        s->serialize(size_, cstr_);


        return s->buffer;
    }

    static String* deserialize(const char* msg) {
      Deserialize* d = new Deserialize();
      size_t size = d->deserialize_size_t(msg);
      char* vals = d->deserialize(msg + sizeof(size_t), size);

      String* str = new String(vals, size);
